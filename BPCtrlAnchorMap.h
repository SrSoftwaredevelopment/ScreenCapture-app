  ...
  void Initialize(HWND hWndParent, DWORD dwFlags/*=0*/) {
    ...
    if (dwFlags & ANIF_CALCSIZE) {
      ...
      for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
        ...
        if (m_Ctrl[iCtrl].rect.right > (double)rcMaxBR.right) rcMaxBR.right = (LONG)m_Ctrl[iCtrl].rect.right;
        if (m_Ctrl[iCtrl].rect.bottom > (double)rcMaxBR.bottom) rcMaxBR.bottom = (LONG)m_Ctrl[iCtrl].rect.bottom;
  ...
  void HandleAnchors(RECT *pRectParent) {
    ...
    for (iCtrl=0; iCtrl < m_nCtrlCount; iCtrl++) {
      ...
      if (bChanged!=FALSE) {
		...
		if ( pCtrl->nFlags & ANF_RIGHT )
			szCtrl.cx = pCtrl->rect.right-pCtrl->rect.left;
		else
			szCtrl.cx = pCtrl->rect.right-pCtrl->rect.left;

		//if ( g_bFullScreen )
		//	szCtrl.cy = pCtrl->rect.bottom-pCtrl->rect.top + 20;
		//else
			szCtrl.cy = pCtrl->rect.bottom-pCtrl->rect.top;
