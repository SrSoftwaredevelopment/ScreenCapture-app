...
CxSkinButton::CxSkinButton()
{
	...
	m_TextColor=RGB(255, 255, 255); // default button text color
...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	RECT tr={r.left+m_FocusRectMargin+2,r.top ,r.right-m_FocusRectMargin-2,r.bottom - 2};
	...
	{
		if (!GetCheck())
			pDC->DrawIcon(13, 13, m_hIcon1);
		else
			pDC->DrawIcon(13, 13, m_hIcon2);
	}
}
/////////////////////////////////////////////////////////////////////////////
...
void CxSkinButton::SetSkin(HBITMAP normal, HBITMAP down, HBITMAP over, UINT disabled, UINT focus, UINT mask, short drawmode, short border, short margin)
{
	...
	m_bNormal.Attach(normal);
	m_bDown.Attach(down);
	m_bOver.Attach(over);
	m_bFocus.Attach(over);
	m_bDisabled.Attach(m_bNormal);
